...
> **Актуальная версия:** `docs/prd-v2.md` (v2.0, 2026-09-18) — смещает приоритет на credibility и техдолг; измерение воронки (PostHog) переведено в deferred (REQ-027). Часть статусов ниже (`[DONE]`) уточнена в v2.

> **Статус реализации (уточнено 2026-09-18 по итогам v2):** статусы `[DONE]` из v1 по REQ-009 (тесты) и REQ-011 (консолидация/keepalive) были **завышены** — тесты не запускались в CI, а пустые keepalive-коммиты жили в трёх местах. Оба закрыты по-настоящему в v2: pytest+ruff в CI (`.github/workflows/ci.yml`), keepalive удалён полностью, `enable_keepalive: false` у RSS-workflow. Всё, что осталось по v1, ведётся в `docs/prd-v2.md` (REQ-020…REQ-028).
...
- **Статус:** рекомендация — вариант (C) оба, реализация начата
...
- **Реализовано (2026-09-07):** UTM `utm_campaign=connect` добавлены на LinkedIn и Blog в секции Connect (Portfolio/Telegram уже имели). Email — mailto, UTM не применим → покрывается ручным учётом.
- **Ручной учёт (процесс):** при входящем контакте от рекрутера/найм-менеджера логировать в PostHog событие `recruiter_contact` с properties: `{source: "linkedin"|"email"|"telegram"|"other", date, company, role}`. Раз в неделю — 5 минут на учёт.
- **Владелец:** Nikita
- **Дедлайн:** конец Phase 2
- **Влияние:** High (North Star метрика)

#### Q2: Social proof — откуда цитаты?
- **Статус:** рекомендация — вариант (A), шаблон готов
- **Варианты:** (A) коллеги по прошлым проектам, (B) рекрутеры из интервью, (C) отложить до получения.
- **Рекомендация:** (A) — коллеги по прошлым проектам (volta-banking, supabase-product-analytics): они видели работу и могут подтвердить навыки. Рекрутеры из интервью — вторичный источник.
- **Шаблон запроса (2 вопроса, минимум усилий для коллеги):**
  1. «Какой результат нашей совместной работы ты бы выделил?» (конкретика, не общие слова)
  2. «Можно ли использовать твою цитату в моём GitHub-профиле с именем и ролью?» (разрешение обязательно)
- **Формат цитаты в README:** `«…» — Имя, Роль (Компания)` под featured project.
- **Владелец:** Nikita
- **Дедлайн:** конец Phase 2
- **Влияние:** Medium

#### Q3: PostHog-проект — отдельный или в существующий?
- **Статус:** рекомендация — вариант (A), создание через UI
...
- **Создание (MCP не даёт create-инструмент, только UI/API):** app.posthog.com → Settings → Projects → New project → name `NikitaBoyarkin Profile` → timezone Europe/Moscow. Затем взять `project_api_key` (phc_...) для отправки событий.
- **Структура событий (после создания проекта):**
  - `profile_link_click` — клики по ссылкам README (через редирект-сервис или аналитику приёмника)
  - `cv_download` — загрузки CV
  - `recruiter_contact` — ручной учёт контактов (Q1)
- **Владелец:** Nikita
